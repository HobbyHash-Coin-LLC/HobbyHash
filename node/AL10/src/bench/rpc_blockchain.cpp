...
static void BlockToJson(benchmark::Bench& bench, TxVerbosity verbosity)
{
    ...
    const Consensus::Params& consensus_params{data.testing_setup->m_node.chainman->GetParams().GetConsensus()};
    const uint256 pow_limit{consensus_params.powLimit};
    bench.run([&] {
        auto univalue = blockToJSON(data.testing_setup->m_node.chainman->m_blockman, data.block, data.blockindex, data.blockindex, verbosity, pow_limit, consensus_params);
...
static void BlockToJsonVerboseWrite(benchmark::Bench& bench)
{
    ...
    const Consensus::Params& consensus_params{data.testing_setup->m_node.chainman->GetParams().GetConsensus()};
    const uint256 pow_limit{consensus_params.powLimit};
    auto univalue = blockToJSON(data.testing_setup->m_node.chainman->m_blockman, data.block, data.blockindex, data.blockindex, TxVerbosity::SHOW_DETAILS_AND_PREVOUT, pow_limit, consensus_params);
    bench.run([&] {
