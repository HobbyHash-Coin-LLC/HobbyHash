 ...
 *
 * Uses the per-algo PoW limit (RandomX ease limit when the block is RandomX;
 * otherwise the shared SHA/KawPow powLimit). Passing the SHA powLimit alone
 * breaks getblockchaininfo / getblockheader when the tip is an eased RandomX
 * block whose nBits target is above the SHA floor.
 *
