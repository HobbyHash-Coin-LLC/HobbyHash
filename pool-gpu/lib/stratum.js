...
var StratumClient = function(options){
    ...
    function handleEthGetWork(message) {
        ...
        var jobParams = (typeof options.getJobParamsForClient === 'function')
            ? options.getJobParamsForClient(_this, null)
            : options.getJobParams();
        if (!jobParams || jobParams.length < 7) {
            sendJson({
                id: message.id,
                jsonrpc: '2.0',
...
var StratumServer = exports.Server = function StratumServer(options, authorizeFn){
    ...
    this.broadcastMiningJobs = function(jobParams){
        for (var clientId in stratumClients) {
            ...
            if (typeof options.getJobParamsForClient === 'function') {
                var clientParams = options.getJobParamsForClient(client, jobParams);
                if (clientParams)
                    params = clientParams;
            }
            client.sendMiningJob(params);
        }
