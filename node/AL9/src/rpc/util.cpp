...
{
    // RandomX may use an easier powLimit after nRandomXEaseActivationHeight.
    // SHA/KawPow keep the shared params.powLimit. Using the SHA floor alone makes
    // DeriveTarget fail on RandomX tips and aborts getblockchaininfo (wallet stuck
    // on "Waiting for RPC" after an otherwise instant restart).
    uint256 pow_limit = params.powLimit;
    if (AlgoAtIndex(&blockindex, params) == PowAlgo::RANDOMX) {
        pow_limit = RandomXPowLimit(blockindex.nHeight, params);
    }
    if (auto target = DeriveTarget(blockindex.nBits, pow_limit)) {
        return ArithToUint256(*target);
    }
    // Display-only fallback: decode compact nBits without the pow_limit clamp so
    // RPC never returns "Internal bug detected" for a tip that is already on disk.
    bool fNegative = false;
    bool fOverflow = false;
    arith_uint256 bn;
    bn.SetCompact(blockindex.nBits, &fNegative, &fOverflow);
    if (fNegative || fOverflow || bn == 0) {
        return uint256{};
    }
    return ArithToUint256(bn);
}
