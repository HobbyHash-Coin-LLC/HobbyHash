...
void CBlockHeaderAndShortTxIDs::DeserializeFromNetwork(DataStream& stream, int nHeight,
                                                       const Consensus::Params& params)
{
    ReadBlockHeaderFromNetwork(stream, header, nHeight, params);
    DeserializePayloadFromNetwork(stream);
}

void CBlockHeaderAndShortTxIDs::DeserializePayloadFromNetwork(DataStream& stream)
{
    stream >> nonce;
    stream >> Using<VectorFormatter<CustomUintFormatter<SHORTTXIDS_LENGTH>>>(shorttxids);
    stream >> prefilledtxn;
