...
// Shared keep-alive agent so repeated RPC polls (getblocktemplate, etc.) reuse a
// small pool of persistent TCP connections to the coin daemon instead of opening
// and immediately closing a fresh socket per call. Without this, high-frequency
// polling floods the daemon's RPC port with short-lived connections that pile up
// in TIME-WAIT and add needless connection-setup load.
var rpcKeepAliveAgent = new http.Agent({
    keepAlive: true,
