#!/usr/bin/env node
/**
 * HOBC V5 preview GPU stratum (regtest backend) for wallet testing.
 * Based on kawpow_personal_stratum_server (KPSS).
 */
const fs = require('fs');
...
const pool = Stratum.createPool({
  ...
  console.log(`Authorize ${workerName} @ preview pool (pass=${password || 'x'}) v=${version} en1=${extraNonce1}`);
  callback({ error: null, authorized: true, disconnect: false });
});

pool.on('share', (isValidShare, isValidBlock, data) => {
  if (isValidBlock) {
    console.log('Block found by GPU miner');
  } else if (isValidShare) {
    console.log('Valid share');
  } else {
    console.log('Share event (validation may fail until getkawpowhash RPC is added)');
  }
  if (data) {
...
console.log(`Starting HOBC V5 preview GPU stratum on 0.0.0.0:${STRATUM_PORT} (RPC ${rpcPort}) pool=${poolAddress}`);
pool.start();
