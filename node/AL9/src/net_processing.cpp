...
bool PeerManagerImpl::CheckHeadersPoW(const std::vector<CBlockHeader>& headers, Peer& peer)
{
    ...
    if (!headers.empty()) {
        LOCK(cs_main);
        const CBlockIndex* prev_index{m_chainman.m_blockman.LookupBlockIndex(headers[0].hashPrevBlock)};
        if (prev_index) {
            prev_height = prev_index->nHeight;
        }
    }

    // Do these headers have proof-of-work matching what's claimed?
    if (!HasValidProofOfWork(headers, m_chainparams.GetConsensus(), prev_height)) {
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        int cmpct_height = -1;
        try {
            vRecv >> cmpctblock.header.nVersion >> cmpctblock.header.hashPrevBlock;
            {
                const CBlockIndex* prev_block = WITH_LOCK(m_chainman.GetMutex(), return m_chainman.m_blockman.LookupBlockIndex(cmpctblock.header.hashPrevBlock));
                if (prev_block) {
                    cmpct_height = prev_block->nHeight + 1;
                }
            }
            vRecv >> cmpctblock.header.hashMerkleRoot >> cmpctblock.header.nTime >> cmpctblock.header.nBits;
            ReadBlockHeaderNonceFields(vRecv, cmpctblock.header, cmpct_height, m_chainparams.GetConsensus());
            cmpctblock.DeserializePayloadFromNetwork(vRecv);
    ...
    if (msg_type == NetMsgType::HEADERS)
    {
        ...
        const Consensus::Params& consensusParams = m_chainparams.GetConsensus();
        int header_base_height = -1;
        for (unsigned int n = 0; n < nCount; n++) {
            if (n == 0) {
                vRecv >> headers[n].nVersion >> headers[n].hashPrevBlock >> headers[n].hashMerkleRoot
                    >> headers[n].nTime >> headers[n].nBits;
                const CBlockIndex* prev_block = WITH_LOCK(m_chainman.GetMutex(), return m_chainman.m_blockman.LookupBlockIndex(headers[n].hashPrevBlock));
                if (prev_block) {
                    header_base_height = prev_block->nHeight;
                }
                ReadBlockHeaderNonceFields(vRecv, headers[n],
                                           header_base_height >= 0 ? header_base_height + 1 : -1,
                                           consensusParams);
            } else {
                ReadBlockHeaderFromNetwork(vRecv, headers[n],
                                           header_base_height >= 0 ? header_base_height + static_cast<int>(n) + 1 : -1,
                                           consensusParams);
            }
    ...
    if (msg_type == NetMsgType::BLOCK)
    {
        ...
        const std::vector<std::byte> block_data(vRecv.begin(), vRecv.end());
        int block_height = -1;
        if (block_data.size() >= 36) {
            ...
            const CBlockIndex* prev_block = WITH_LOCK(m_chainman.GetMutex(), return m_chainman.m_blockman.LookupBlockIndex(hashPrevBlock));
            if (prev_block) {
        ...
        std::shared_ptr<CBlock> pblock = std::make_shared<CBlock>();
        if (!DeserializeBlockFromBytes(*pblock, block_data, block_height, m_chainparams.GetConsensus())) {
            Misbehaving(peer, "invalid block");
            return;
        }
