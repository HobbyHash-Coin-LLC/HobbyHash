...
bool BlockTreeDB::LoadBlockIndexGuts(const Consensus::Params& consensusParams, std::function<CBlockIndex*(const uint256&)> insertBlockIndex, const util::SignalInterrupt& interrupt)
{
    ...
    // Ensure CDiskBlockIndex deserialization sees the correct consensus params for
    // this network. Without an explicit scope it falls back to default (mainnet)
    // params, which is wrong on testnet/regtest and historically dangled. The disk
    // header layout depends on these params (KawPow nNonce64/mixHash on GPU heights),
    // so loading with the wrong params yields a wrong block hash and a spurious
    // CheckBlockProofOfWork failure. Height is unused by CDiskBlockIndex (it keys off
    // the on-disk nHeight); only the params matter here.
    BlockHeaderUnserializeScope disk_index_scope(-1, consensusParams);
    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                // Use the index-load PoW check (structural KawPow, full SHA). Full
                // progpow::verify on every GPU index entry stalls wallet cold starts.
                if (!CheckBlockProofOfWorkForIndexLoad(pindexNew->GetBlockHeader(), pindexNew->nHeight, consensusParams)) {
                    LogError("%s: CheckBlockProofOfWork failed: %s\n", __func__, pindexNew->ToString());
                    return false;
                }
...
CBlockIndex* BlockManager::AddToBlockIndex(const CBlockHeader& block, CBlockIndex*& best_header)
{
    ...
    UpdateDualPowBitsOnIndex(*pindexNew, block, GetConsensus());
...
void BlockManager::WriteBlockIndexDB()
{
    ...
    // Serialize CDiskBlockIndex under the ACTIVE chain's consensus params so the height-gated
    // extended-field persistence (KawPow bit17, RandomX bit18 nNonce64/mixHash) matches the read
    // path in LoadBlockIndexGuts. Without this the writer falls back to DefaultUnserializeParams()
    // (mainnet), which on regtest/testnet skips RandomX fields the reader then tries to read
    // ("failed to read value"). Mirrors disk_index_scope in LoadBlockIndexGuts.
    BlockHeaderUnserializeScope write_index_scope(-1, GetConsensus());
...
bool BlockManager::ReadBlock(CBlock& block, const FlatFilePos& pos, const std::optional<uint256>& expected_hash) const
{
    ...
    // Pool software may set version bit 17 on legacy SHA blocks stored with an 80-byte header.
    if (block_data->size() >= sizeof(int32_t)) {
        const int32_t stored_version = ReadLE32(reinterpret_cast<const unsigned char*>(block_data->data()));
        const bool legacy_bit17 = (stored_version & BLOCK_VERSION_GPU_KAWPOW) != 0;
        const bool hash_mismatch = expected_hash && block_hash != *expected_hash;
        if (legacy_bit17 && (block.vtx.empty() || hash_mismatch)) {
            block.SetNull();
            if (!DeserializeBlockFromBytes(block, *block_data, /*nHeight=*/0, GetConsensus()) ||
                block.vtx.empty()) {
                return false;
            }
...
bool BlockManager::ReadBlock(CBlock& block, const CBlockIndex& index) const
{
    ...
    try {
        if (!DeserializeBlockFromBytes(block, *block_data, index.nHeight, GetConsensus())) {
            return false;
        }
    } catch (const std::exception& e) {
        LogError("Deserialize or I/O error - %s at %s while reading block", e.what(), block_pos.ToString());
        return false;
    }

    if (index.nTx > 0 && block.vtx.empty()) {
        LogError("Empty vtx at height %d while reading block at %s", index.nHeight, block_pos.ToString());
        return false;
    }

    const auto block_hash{block.GetHash()};
    if (!CheckBlockProofOfWork(block, index.nHeight, GetConsensus())) {
        LogError("Errors in block header at %s while reading block", block_pos.ToString());
        return false;
    }
    ...
    if (block_hash != expected_hash) {
        LogError("GetHash() doesn't match index at %s while reading block (%s != %s)",
                 block_pos.ToString(), block_hash.ToString(), expected_hash.ToString());
        return false;
    }
