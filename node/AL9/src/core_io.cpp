...
bool DecodeHexBlk(CBlock& block, const std::string& strHexBlk)
{
    ...
    if (blockData.size() < sizeof(int32_t)) {
        return false;
    }

    const int32_t version = ReadLE32(blockData.data());
    if ((version & BLOCK_VERSION_GPU_KAWPOW) == 0) {
        return false;
    }

    const Consensus::Params& params = BlockHeaderUnserializeParams();
    int gpu_height = params.nDualPowActivationHeight;
    while (gpu_height > 0 && !IsGpuBlockHeight(gpu_height, params)) {
        ++gpu_height;
    }
    if (gpu_height <= 0) {
        return false;
    ...
    try {
        BlockHeaderUnserializeScope scope(gpu_height, params);
        SpanReader{blockData} >> TX_WITH_WITNESS(block);
        return true;
    } catch (const std::exception&) {
        return false;
