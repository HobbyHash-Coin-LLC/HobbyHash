    ...
    case RESTResponseFormat::JSON: {
        if (tx_verbosity) {
            ...
            if (!DeserializeBlockFromBytes(block, *block_data, pblockindex->nHeight, chainman.GetConsensus())) {
                return RESTERR(req, HTTP_INTERNAL_SERVER_ERROR, "Block deserialization failed");
            }
            UniValue objBlock = blockToJSON(chainman.m_blockman, block, *tip, *pblockindex, *tx_verbosity, chainman.GetConsensus().powLimit, chainman.GetConsensus());
