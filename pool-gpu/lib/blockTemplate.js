...
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, reward, recipients, poolAddress){
    ...
    /* Per-client coinbase identity (rig/worker TLVs). Merkle + header_hash change with the
     * coinbase, so KawPow jobs must be minted per authorized worker. */
    this.bindMiner = function (workerName, userAgent) {
        ...
        if (genTx && typeof genTx.toString === 'function' && genTx.toString('hex') !== genTx)
            genTx = genTx.toString('hex');
        else
            genTx = String(genTx);
        var genTxHash = transactions.txHash();
        var merkleRoot = merkle.getRoot(rpcData, genTxHash);
        var header_hash = util.kawPowHeaderHashHex(rpcData, merkleRoot);
        var seedhash = util.kawPowSeedHashHex(rpcData.height);
        var bind = {
            genTx: genTx,
            genTxHash: genTxHash,
    ...
    this.getJobParamsForMiner = function (workerName, userAgent) {
        return this.bindMiner(workerName, userAgent).jobParams.slice();
    };

    this.findBindByHeaderHash = function (headerHash) {
        var normalized = String(headerHash || '').toLowerCase().replace(/^0x/i, '');
        var key;
        if (!normalized)
            return null;
        for (key in minerBindCache) {
            if (minerBindCache[key].headerHash === normalized)
                return minerBindCache[key];
        }
        return null;
    };

    this.findBindByHeaderPrefix = function (prefix) {
        var normalized = String(prefix || '').toLowerCase().replace(/^0x/i, '');
        var key;
        if (!normalized)
            return null;
        for (key in minerBindCache) {
            if (minerBindCache[key].headerHash.slice(0, normalized.length) === normalized)
                return minerBindCache[key];
        }
        return null;
    };
