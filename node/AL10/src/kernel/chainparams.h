...
public:
    ...
    struct RegTestOptions {
        ...
        //! Test-only: enable per-algo LWMA retargeting on regtest (default off keeps
        //! fPowNoRetargeting=true so existing regtest behavior is unchanged). Used by
        //! the V6 multi-algo timing burn-in harness so difficulty floats to hold the
        //! per-algo target spacing with real external miners.
        bool lwma_retarget{false};
    };
