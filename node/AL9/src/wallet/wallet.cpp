...
bool CWallet::SignTransaction(CMutableTransaction& tx) const
{
    ...
    return SignTransaction(tx, fail_reason);
}

bool CWallet::SignTransaction(CMutableTransaction& tx, bilingual_str& fail_reason) const
{
    AssertLockHeld(cs_wallet);
    fail_reason.clear();

    // Prefer the live chain UTXO set for each selected input. Some post-upgrade
    // wallets can carry stale funding-output records in mapWallet; signing those
    // stale scriptPubKey/amount pairs fails with "Witness program hash mismatch".
    // Fall back to mapWallet only when the live UTXO is unavailable.
    std::map<COutPoint, Coin> coins;
    for (const auto& input : tx.vin) {
        coins[input.prevout] = Coin();
    }
    chain().findCoins(coins);

    for (const auto& input : tx.vin) {
        const COutPoint& outpoint = input.prevout;
        Coin& coin = coins.at(outpoint);
        const auto mi = mapWallet.find(outpoint.hash);

        if (!coin.out.IsNull()) {
            if (mi != mapWallet.end() && outpoint.n < mi->second.tx->vout.size()) {
                const CWalletTx& wtx = mi->second;
                coin.nHeight = wtx.state<TxStateConfirmed>() ? wtx.state<TxStateConfirmed>()->confirmed_block_height : coin.nHeight;
        ...
        if (mi == mapWallet.end() || outpoint.n >= mi->second.tx->vout.size()) {
            fail_reason = strprintf(_("Signing transaction failed: wallet has no record for input %s:%u. Try reloading the wallet with a rescan from the Backup page or restore your wallet backup."), outpoint.hash.GetHex(), outpoint.n);
            return false;
        }

        const CWalletTx& wtx = mi->second;
        const int prev_height = wtx.state<TxStateConfirmed>() ? wtx.state<TxStateConfirmed>()->confirmed_block_height : 0;
        coin = Coin(wtx.tx->vout[outpoint.n], prev_height, wtx.IsCoinBase());
    }

    std::map<int, bilingual_str> input_errors;
    if (!SignTransaction(tx, coins, SIGHASH_DEFAULT, input_errors)) {
        fail_reason = _("Signing transaction failed");
        if (!input_errors.empty()) {
            fail_reason += Untranslated(": ") + input_errors.begin()->second;
        }
        return false;
    }
    return true;
...
{
    const int sign_at_height = GetLastBlockHeight() + 1;
    auto transaction_complete = [&]() {
        input_errors.clear();
        ...
        const auto replay = ReplayForkForHeight(sign_at_height, Params().GetConsensus());
        for (unsigned int i = 0; i < tx.vin.size(); ++i) {
            const CTxIn& txin = tx.vin[i];
            const Coin& coin = coins.at(txin.prevout);
            ScriptError serror = SCRIPT_ERR_OK;
            if (!VerifyScript(txin.scriptSig, coin.out.scriptPubKey, &txin.scriptWitness, STANDARD_SCRIPT_VERIFY_FLAGS,
                              TransactionSignatureChecker(&tx_const, i, coin.out.nValue, txdata, MissingDataBehavior::FAIL, replay.fork_id, replay.require_sighash), &serror)) {
    ...
    // Let each ScriptPubKeyMan add signatures for the inputs it owns. Mixed
    // descriptor wallets can spend receive-chain and change-chain coins in the
    // same transaction, so no single manager is required to complete the whole
    // transaction by itself.
    for (ScriptPubKeyMan* spk_man : GetAllScriptPubKeyMans()) {
        input_errors.clear();
        if (spk_man->SignTransaction(tx, coins, sighash, input_errors, sign_at_height)) {
            return true;
        }
...
std::optional<PSBTError> CWallet::FillPSBT(PartiallySignedTransaction& psbtx, bool& complete, std::optional<int> sighash_type, bool sign, bool bip32derivs, size_t * n_signed, bool finalize) const
{
    ...
    for (ScriptPubKeyMan* spk_man : GetAllScriptPubKeyMans()) {
        ...
        const auto error{spk_man->FillPSBT(psbtx, txdata, sighash_type, sign, bip32derivs, &n_signed_this_spkm, finalize, sign_at_height)};
        if (error) {
            return error;
        }
