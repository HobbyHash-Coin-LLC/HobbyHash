...
private:
    ...
    uint32_t m_replay_fork_id;
    bool m_require_fork_sighash;

public:
    CScriptCheck(const CTxOut& outIn, const CTransaction& txToIn, SignatureCache& signature_cache, unsigned int nInIn, script_verify_flags flags, bool cacheIn, PrecomputedTransactionData* txdataIn, uint32_t replay_fork_id = 0, bool require_fork_sighash = false) :
        m_tx_out(outIn), ptxTo(&txToIn), nIn(nInIn), m_flags(flags), cacheStore(cacheIn), txdata(txdataIn), m_signature_cache(&signature_cache), m_replay_fork_id(replay_fork_id), m_require_fork_sighash(require_fork_sighash) { }
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true, bool fCheckMerkleRoot = true, int nHeight = -1);
 ...
 *  When prev_height >= 0, header i is validated at height prev_height + 1 + i (required
 *  for KawPow headers past the GPU activation height). */
bool HasValidProofOfWork(std::span<const CBlockHeader> headers, const Consensus::Params& consensusParams, int prev_height = -1);
