...
public:
    ...
    /** Height-aware P2P deserialize (header nonce/mixHash depends on dual-PoW height). */
    void DeserializeFromNetwork(DataStream& stream, int nHeight, const Consensus::Params& params);

    /** Deserialize nonce/shorttxids/prefilledtxn after header fields are read. */
    void DeserializePayloadFromNetwork(DataStream& stream);
};
