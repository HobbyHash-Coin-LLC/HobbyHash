...
bool CheckInputScripts(const CTransaction& tx, TxValidationState& state,
                       ...
                       uint32_t replay_fork_id = 0, bool require_fork_sighash = false)
                ...
                EXCLUSIVE_LOCKS_REQUIRED(cs_main, pool.cs)
{
    ...
    return CheckInputScripts(tx, state, view, flags, /* cacheSigStore= */ true, /* cacheFullScriptStore= */ true, txdata, validation_cache, nullptr, replay_fork_id, require_fork_sighash);
}
...
bool MemPoolAccept::PolicyScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    const auto replay = ReplayForkForHeight(next_height, consensusParams);

    // Check input scripts and signatures.
    // This is done last to help prevent CPU exhaustion denial-of-service attacks.
    if (!CheckInputScripts(tx, state, m_view, scriptVerifyFlags, true, false, ws.m_precomputed_txdata, GetValidationCache(), nullptr, replay.fork_id, replay.require_sighash)) {
...
bool MemPoolAccept::ConsensusScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    if (!CheckInputsFromMempoolAndCache(tx, state, m_view, m_pool, currentBlockScriptVerifyFlags,
                                        ...
                                        replay.fork_id, replay.require_sighash)) {
...
{
    return Consensus::GetBlockSubsidy(nHeight, consensusParams, nullptr);
}

CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    return Consensus::GetBlockSubsidy(nHeight, consensusParams, pindexPrev);
}
...
std::optional<std::pair<ScriptError, std::string>> CScriptCheck::operator()() {
    ...
    if (VerifyScript(scriptSig, m_tx_out.scriptPubKey, witness, m_flags, CachingTransactionSignatureChecker(ptxTo, nIn, m_tx_out.nValue, cacheStore, *m_signature_cache, *txdata, m_replay_fork_id, m_require_fork_sighash), &error)) {
        return std::nullopt;
    } else {
...
bool CheckInputScripts(const CTransaction& tx, TxValidationState& state,
                       ...
                       uint32_t replay_fork_id, bool require_fork_sighash)
{
    ...
    // HobbyHash replay-fork sighash is part of the script result: the same tx+flags can
    // be valid under one fork id and invalid under another (the fork id is committed to
    // by the signature). Bind the cache entry to (replay_fork_id, require_fork_sighash)
    // so a result computed under one fork id is never reused across a fork-id boundary
    // (e.g. a tx signed for the pre-race id validated in mempool must NOT satisfy the
    // block-connect check that requires the post-race id). Without this, a warm cache
    // accepts a block that a cold reindex correctly rejects -> latent chain split.
    {
        const unsigned char rfs = require_fork_sighash ? 1 : 0;
        hasher.Write((const unsigned char*)&replay_fork_id, sizeof(replay_fork_id)).Write(&rfs, 1);
    }
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        // Verify signature
        CScriptCheck check(txdata.m_spent_outputs[i], tx, validation_cache.m_signature_cache, i, flags, cacheSigStore, &txdata, replay_fork_id, require_fork_sighash);
        if (pvChecks) {
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    const auto replay = ReplayForkForHeight(pindex->nHeight, consensusParams);
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase() && fScriptChecks)
        {
            ...
            if (control) {
                ...
                tx_ok = CheckInputScripts(tx, tx_state, view, flags, fCacheResults, fCacheResults, txsdata[i], m_chainman.m_validation_cache, &vChecks, replay.fork_id, replay.require_sighash);
                if (tx_ok) control->Add(std::move(vChecks));
            } else {
                tx_ok = CheckInputScripts(tx, tx_state, view, flags, fCacheResults, fCacheResults, txsdata[i], m_chainman.m_validation_cache, nullptr, replay.fork_id, replay.require_sighash);
            }
            if (!tx_ok) {
                // Any transaction validation failure in ConnectBlock is a block consensus failure
                state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
...
static bool CheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true, int nHeight = -1)
{
    if (fCheckPOW) {
        // Route to the multi-algo proof-of-work checker whenever EITHER the legacy dual-PoW
        // (SHA/KawPow) window OR the V6 multi-algo race is active at this height. Using only
        // DualPowActive here mis-validated RandomX/KawPow race blocks as plain SHA (hashing the
        // 80-byte legacy prefix) on any chain where the race activates before dual-PoW (e.g.
        // regtest: race=200, dual=8000), intermittently accepting bad-PoW blocks and rejecting
        // valid ones as "high-hash". MultiAlgoRaceActive is height-gated and inert below activation,
        // so pre-activation and historical validation are byte-for-byte unchanged.
        const bool use_dual = nHeight >= 0 &&
            (DualPowActive(nHeight, consensusParams) || MultiAlgoRaceActive(nHeight, consensusParams));
        if (use_dual) {
            if (!CheckBlockProofOfWork(block, nHeight, consensusParams)) {
...
bool HasValidProofOfWork(std::span<const CBlockHeader> headers, const Consensus::Params& consensusParams, int prev_height)
{
    for (size_t i = 0; i < headers.size(); ++i) {
        const CBlockHeader& header = headers[i];
        if (prev_height >= 0) {
            BlockValidationState state;
            if (!CheckBlockHeader(header, state, consensusParams, /*fCheckPOW=*/true, prev_height + 1 + static_cast<int>(i))) {
                return false;
            }
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    // Check difficulty matches expectation (actual PoW checked in CheckBlockHeader).
    const Consensus::Params& consensusParams = chainman.GetConsensus();
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev)
{
    ...
    // V6 multi-algo race: HOBC coinbase telemetry marker is consensus-required at/after the
    // activation height (format-required enforcement). Below activation the coinbase is never
    // inspected for a marker, so this is byte-identical to v31.0.8 for every historical block.
    // The marker's algo field must match the header-derived algo (dual-bound discriminator).
    {
        ...
        if (MultiAlgoRaceActive(nHeight, params)) {
            if (block.vtx.empty() || block.vtx[0]->vin.empty()) {
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-missing", "missing coinbase for marker");
            }
            const CScript& cb_ss = block.vtx[0]->vin[0].scriptSig;
            const std::span<const unsigned char> cb_bytes{
                reinterpret_cast<const unsigned char*>(cb_ss.data()), cb_ss.size()};
            HobcTelemetryMarker marker;
            if (!ParseHobcTelemetryMarker(cb_bytes, marker)) {
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-hobc-marker",
                                     "missing or malformed HOBC coinbase telemetry marker");
            }
            const PowAlgo hdr_algo = AlgoFromHeader(block, nHeight, params);
            if (static_cast<int>(marker.algo) != static_cast<int>(hdr_algo)) {
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-hobc-marker-algo",
                                     strprintf("HOBC marker algo %d != header algo %d",
                                               static_cast<int>(marker.algo), static_cast<int>(hdr_algo)));
            }
        }
    }
...
bool ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, CBlockIndex** ppindex, bool min_pow_checked)
{
    ...
    if (hash != GetConsensus().hashGenesisBlock) {
        ...
        if (!CheckBlockHeader(block, state, GetConsensus(), true, pindexPrev->nHeight + 1)) {
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        BlockMap::iterator mi = m_blockman.m_block_index.find(block->hashPrevBlock);
        if (mi != m_blockman.m_block_index.end()) {
            block_height = mi->second.nHeight + 1;
        }
        bool ret = CheckBlock(*block, state, GetConsensus(), true, true, block_height);
        if (ret) {
            // Store to disk
