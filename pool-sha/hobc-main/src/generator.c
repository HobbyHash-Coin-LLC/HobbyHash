...
retry:
	...
	if (cmdmatch(buf, "getbase")) {
		if (!gen_gbtbase(cs, &gbt)) {
			...
			goto reconnect;
		} else if (gbt.sha_idle_gpu_window) {
			LOGNOTICE("Generator idle during GPU KawPow window");
			clear_gbtbase(&gbt);
			send_unix_msg(umsg->sockd, "gpu_idle");
			sleep(2);
			goto retry;
		} else {
...
bool generator_load_address_script(ckpool_t *ckp, const char *addr, char *script_bin, int *script_len)
{
	gdata_t *gdata = ckp->gdata;
	server_instance_t *si;
	connsock_t *cs;
	char rpc_req[256];
	json_t *val = NULL, *res_val = NULL, *valid_val = NULL, *spk_val = NULL;
	const char *script_hex = NULL;
	bool ret = false;

	if (unlikely(!addr || !script_bin || !script_len))
		return false;

	si = gdata ? gdata->current_si : NULL;
	if (unlikely(!si)) {
		LOGWARNING("No live current server in generator_load_address_script");
		return false;
	}
	cs = &si->cs;
	snprintf(rpc_req, sizeof(rpc_req), "{\"method\": \"validateaddress\", \"params\": [\"%s\"]}\n", addr);
	val = json_rpc_call(cs, rpc_req);
	if (!val)
		return false;
	res_val = json_object_get(val, "result");
	valid_val = res_val ? json_object_get(res_val, "isvalid") : NULL;
	if (!valid_val || !json_is_true(valid_val))
		goto out;
	spk_val = json_object_get(res_val, "scriptPubKey");
	script_hex = spk_val ? json_string_value(spk_val) : NULL;
	if (!script_hex || !script_hex[0])
		goto out;
	*script_len = strlen(script_hex) / 2;
	if (*script_len <= 0 || *script_len > 25)
		goto out;
	hex2bin(script_bin, script_hex, *script_len);
	ret = true;
out:
	if (val)
		json_decref(val);
