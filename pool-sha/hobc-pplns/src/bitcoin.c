...
static bool validate_legacy_base58_address(const char *address, int len)
{
	int i, j;
...
bool validate_address(connsock_t *cs, const char *address)
{
	...
	len = strlen(address);
	if (!strncmp(address, "hobc1", 5)) {
		if (len < 11 || len > 90 || !looks_like_hobc_bech32(address)) {
			LOGDEBUG("Invalid hobc1 bech32 address passed to validate_address");
			return ret;
		}
	} else if (!validate_legacy_base58_address(address, len)) {
		LOGDEBUG("Invalid legacy address passed to validate_address");
		return ret;
	}
...
bool gen_gbtbase(connsock_t *cs, gbtbase_t *gbt)
{
	...
	/* Always define the fields clear_gbtbase()/callers touch, even on the
	 * early kawpow-idle return below, so a caller that passes an uninitialized
	 * stack gbtbase_t (server_alive, the generator loop) never frees or
	 * json_decrefs a garbage pointer (KawPow-window SEGV fix). */
	gbt->json = NULL;
	gbt->flags = NULL;
	gbt->sha_idle_gpu_window = false;
	...
	{
		const char *powalgo = json_string_value(json_object_get(res_val, "powalgo"));
		if (powalgo && !strcmp(powalgo, "kawpow")) {
			gbt->sha_idle_gpu_window = true;
			ret = true;
			goto out;
		}
	}
