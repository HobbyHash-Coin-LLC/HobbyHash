...
public:
    ...
    /** V5/V6 per-algo difficulty tracks (memory; rebuilt on load if zero). */
    uint32_t nBitsSha{0};
    uint32_t nBitsGpu{0};
    uint32_t nBitsRx{0};

    /** KawPow header fields mirror (memory). */
    uint64_t nNonce64{0};
    uint256 mixHash{};
...
class CDiskBlockIndex : public CBlockIndex
{
     ...
     *
     * 310001: persist KawPow nNonce64/mixHash on GPU block heights.
     **/
    static constexpr int DUMMY_VERSION = 310001;
...
public:
    ...
    SERIALIZE_METHODS(CDiskBlockIndex, obj)
    {
        ...
        const Consensus::Params& disk_params = BlockHeaderUnserializeParams();
        // Extended (nNonce64+mixHash) persistence, split by era so historical pre-race
        // on-disk entries stay byte-identical:
        //   * Pre-race windowed dual-PoW: only genuine GPU/KawPow heights carry ext fields.
        //     A pre-fork SHA block may legally set version bit 17 (BIP320 AsicBoost) WITHOUT
        //     nNonce64/mixHash, so gate on the windowed GPU height, not the raw bit.
        //   * V6 multi-algo race (>= activation): the ext layout is signalled by the
        //     rolling-safe BLOCK_VERSION_V6_EXTENDED bit (bit 0, outside the BIP320 0x1fffe000
        //     roll domain). GBT sets it for BOTH race KawPow (bit 17) and RandomX (bit 18)
        //     solves and never for a race SHA header, matching the wire/blockfile gating in
        //     primitives/block.h. The previous code gated race KawPow on IsGpuBlockHeight(),
        //     which is ALWAYS false in race mode, so race KawPow nNonce64/mixHash were never
        //     persisted and cold index load rebuilt a legacy 80-byte hash that failed PoW
        //     (e.g. GPU block 16816 -> "CheckBlockProofOfWork failed", node could not restart).
        //     Bit-0 gating fixes race KawPow while staying exact for race RandomX (which already
        //     persisted ext fields) and for an AsicBoost-rolled race SHA header (bit 0 clear).
        //     NOTE: a datadir written by the pre-fix binary that already contains race KawPow
        //     index entries must be rebuilt with -reindex when upgrading (the coordinated V6
        //     redeploy does exactly this); the raw blockfile carries the correct 40-byte header.
        bool disk_extended;
        if (MultiAlgoRaceActive(obj.nHeight, disk_params)) {
            disk_extended = (obj.nVersion & BLOCK_VERSION_V6_EXTENDED) != 0;
        } else {
            disk_extended = (obj.nVersion & BLOCK_VERSION_GPU_KAWPOW) != 0
                && DualPowActive(obj.nHeight, disk_params)
                && IsGpuBlockHeight(obj.nHeight, disk_params);
        }
        if (disk_extended) {
