...
/** V4 replay protection: OR SIGHASH_FORKID when signing at/after fork height. */
inline int ReplayAwareSignHashType(int nHashType, uint32_t replay_fork_id)
{
    if (replay_fork_id == 0) {
        return nHashType == SIGHASH_DEFAULT ? SIGHASH_ALL : nHashType;
    }
    const int base = nHashType == SIGHASH_DEFAULT ? SIGHASH_ALL : nHashType;
    return base | SIGHASH_FORKID;
}
...
class MutableTransactionSignatureCreator : public BaseSignatureCreator
{
    ...
    const uint32_t m_replay_fork_id;
    const bool m_require_fork_sighash;

    std::optional<uint256> ComputeSchnorrSignatureHash(const uint256* leaf_hash, SigVersion sigversion) const;

public:
    MutableTransactionSignatureCreator(const CMutableTransaction& tx LIFETIMEBOUND, unsigned int input_idx, const CAmount& amount, int hash_type, uint32_t replay_fork_id = 0, bool require_fork_sighash = false);
    MutableTransactionSignatureCreator(const CMutableTransaction& tx LIFETIMEBOUND, unsigned int input_idx, const CAmount& amount, const PrecomputedTransactionData* txdata, int hash_type, uint32_t replay_fork_id = 0, bool require_fork_sighash = false);
...
bool SignTransaction(CMutableTransaction& mtx, const SigningProvider* provider, const std::map<COutPoint, Coin>& coins, int sighash, std::map<int, bilingual_str>& input_errors, int sign_at_height = -1);
