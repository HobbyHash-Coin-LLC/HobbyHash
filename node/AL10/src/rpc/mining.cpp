        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    const int next_h = tip.nHeight + 1;
    const auto& cp = chainman.GetConsensus();
    if (MultiAlgoRaceActive(next_h, cp)) {
        UniValue algos(UniValue::VARR);
        algos.push_back("sha256");
        algos.push_back("kawpow");
        algos.push_back("randomx");
        obj.pushKV("powalgos", algos);
        obj.pushKV("powalgo", "sha256"); // default; pools select via GBT rules
        obj.pushKV("bits_sha", strprintf("%08x", GetBitsShaAtIndex(&tip, cp)));
        obj.pushKV("bits_gpu", strprintf("%08x", GetBitsGpuAtIndex(&tip, cp)));
        obj.pushKV("bits_rx", strprintf("%08x", GetBitsRxAtIndex(&tip, cp)));
        obj.pushKV("race_active", true);
    } else if (DualPowActive(next_h, cp)) {
        obj.pushKV("powalgo", IsGpuBlockHeight(next_h, cp) ? "kawpow" : "sha256");
        obj.pushKV("bits_sha", strprintf("%08x", GetBitsShaAtIndex(&tip, cp)));
        obj.pushKV("bits_gpu", strprintf("%08x", GetBitsGpuAtIndex(&tip, cp)));
        obj.pushKV("heightmod6", next_h % cp.nDualPowCycleLength);
    } else {
        obj.pushKV("powalgo", "sha256");
    }
    return obj;
},
    };
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Optional GBT rules: "kawpow" / "randomx" / "sha256" select the race-mode algo.
    PowAlgo gbt_algo = PowAlgo::SHA256;
    if (MultiAlgoRaceActive(nNextHeight, consensusParams)) {
        const bool want_kawpow = setClientRules.count("kawpow") > 0;
        const bool want_randomx = setClientRules.count("randomx") > 0;
        const bool want_sha = setClientRules.count("sha256") > 0;
        if (want_kawpow && !want_randomx) gbt_algo = PowAlgo::KAWPOW;
        else if (want_randomx && !want_kawpow) gbt_algo = PowAlgo::RANDOMX;
        else if (want_sha) gbt_algo = PowAlgo::SHA256;
        else gbt_algo = PowAlgo::SHA256;
    } else if (DualPowActive(nNextHeight, consensusParams) && IsGpuBlockHeight(nNextHeight, consensusParams)) {
        gbt_algo = PowAlgo::KAWPOW;
    }
    if (gbt_algo == PowAlgo::KAWPOW) {
        block.nVersion |= CBlockHeader::GPU_KAWPOW_VERSION;
        // V6: mark the extended (nNonce64+mixHash) header with the rolling-safe bit 0 so
        // the node decodes the 40-byte layout deterministically regardless of any AsicBoost
        // bit-rolling. Pools must preserve this bit in the submitted block version.
        block.nVersion |= BLOCK_VERSION_V6_EXTENDED;
        block.nNonce64 = 0;
        block.mixHash.SetNull();
        block.nBits = GetNextWorkRequiredLwmaForAlgo(pindexPrev, &block, PowAlgo::KAWPOW, consensusParams);
    } else if (gbt_algo == PowAlgo::RANDOMX) {
        block.nVersion |= BLOCK_VERSION_CPU_RANDOMX;
        block.nVersion |= BLOCK_VERSION_V6_EXTENDED;
        block.nNonce64 = 0;
        block.mixHash.SetNull();
        block.nBits = GetNextWorkRequiredLwmaForAlgo(pindexPrev, &block, PowAlgo::RANDOMX, consensusParams);
    } else if (MultiAlgoRaceActive(nNextHeight, consensusParams)) {
        block.nBits = GetNextWorkRequiredLwmaForAlgo(pindexPrev, &block, PowAlgo::SHA256, consensusParams);
    }
    ...
    // V6: re-assert the rolling-safe extended-header bit AFTER the BIP9 version-bits
    // loop above (which ANDs/ORs deployment masks into nVersion) so no deployment mask
    // can strip it from the advertised template version. Pools serialize this version
    // verbatim, and the node's submitblock/P2P decode gate on this bit for the 40-byte
    // (nNonce64+mixHash) KawPow/RandomX header layout.
    if (gbt_algo == PowAlgo::KAWPOW || gbt_algo == PowAlgo::RANDOMX) {
        block.nVersion |= BLOCK_VERSION_V6_EXTENDED;
    }
    ...
    if (MultiAlgoRaceActive(nNextHeight, consensusParams)) {
        result.pushKV("powalgo", PowAlgoName(gbt_algo));
        UniValue algos(UniValue::VARR);
        algos.push_back("sha256");
        algos.push_back("kawpow");
        algos.push_back("randomx");
        result.pushKV("powalgos", algos);
        result.pushKV("bits_sha", strprintf("%08x", GetBitsShaAtIndex(pindexPrev, consensusParams)));
        result.pushKV("bits_gpu", strprintf("%08x", GetBitsGpuAtIndex(pindexPrev, consensusParams)));
        result.pushKV("bits_rx", strprintf("%08x", GetBitsRxAtIndex(pindexPrev, consensusParams)));
        result.pushKV("race_active", true);
        if (gbt_algo == PowAlgo::KAWPOW || gbt_algo == PowAlgo::RANDOMX) {
            result.pushKV("noncerange", "0000000000000000ffffffffffffffff");
        }
    } else if (DualPowActive(nNextHeight, consensusParams)) {
        result.pushKV("powalgo", IsGpuBlockHeight(nNextHeight, consensusParams) ? "kawpow" : "sha256");
        result.pushKV("bits_sha", strprintf("%08x", GetBitsShaAtIndex(pindexPrev, consensusParams)));
        result.pushKV("bits_gpu", strprintf("%08x", GetBitsGpuAtIndex(pindexPrev, consensusParams)));
        result.pushKV("heightmod6", nNextHeight % consensusParams.nDualPowCycleLength);
        if (IsGpuBlockHeight(nNextHeight, consensusParams)) {
            result.pushKV("noncerange", "0000000000000000ffffffffffffffff");
        }
    } else {
        result.pushKV("powalgo", "sha256");
    }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Height-aware decode (mirrors the P2P block path). An ASICBoost SHA miner
    // may set version bit 17 (BLOCK_VERSION_GPU_KAWPOW) via BIP320 version
    // rolling; such a block is still a plain 80-byte SHA header. The version-only
    // DecodeHexBlk path would parse it as a 116-byte KawPow header and corrupt
    // it, so a valid solve is rejected as "high-hash". Resolve the target height
    // from the previous block so DeserializeBlockFromBytes selects the correct
    // (SHA 80-byte vs GPU/KawPow) header layout.
    const auto block_data{TryParseHex<std::byte>(request.params[0].get_str())};
    if (!block_data) {
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "Block decode failed");
    }
    int block_height = -1;
    if (block_data->size() >= 36) {
        const auto hash_bytes = std::span{*block_data}.subspan(4, 32);
        const uint256 hashPrevBlock{
            std::span<const unsigned char>{reinterpret_cast<const unsigned char*>(hash_bytes.data()), hash_bytes.size()}};
        const CBlockIndex* prev_block = WITH_LOCK(cs_main, return chainman.m_blockman.LookupBlockIndex(hashPrevBlock));
        if (prev_block) {
            block_height = prev_block->nHeight + 1;
        }
    }
    if (!DeserializeBlockFromBytes(block, *block_data, block_height, chainman.GetParams().GetConsensus())) {
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "Block decode failed");
    }
    {
        LOCK(cs_main);
