...
struct user_instance {
	...
	/* direct_worker_payout: scriptPubKey for this user's HOBC address */
	uchar payout_script[32];
	int payout_script_len;
	bool payout_script_ok;
};
...
struct stratum_instance {
	...
	char *useragent;
	char *workername;
	/* Original mining.authorize workername before fallback/strip rewrite.
	 * Miners keep submitting this string; compare against it on submit. */
	char *auth_workername;
	char *password;
...
struct stratifier_data {
	...
	time_t update_time;
	/* True while the chain is in a GPU/KawPow window and the SHA pool is
	 * intentionally idle. Set on window entry (SHA miners are dropped once)
	 * and cleared when SHA work resumes. */
	bool gpu_idle_window;
...
static void generate_coinbase(const ckpool_t *ckp, workbase_t *wb)
{
	...
	wb->coinb2len = 7;
	if (ckp->hobc_marker) {
		/* HOBC V6: emit the consensus telemetry marker. It replaces btcsig so the coinbase
		 * scriptSig stays within the 100-byte cap. winning_share_diff carries the block's
		 * network difficulty at job time. Marker omits rig (0x05); site≤24 + worker 16
		 * leave ≥1 byte headroom. build_client_coinb2 patches worker in place.
		 * Prefixed with a pushdata length byte; the node parser locates "HOBC" by scan.
		 *
		 * network_diff is normally set in add_base() from headerbin, which is built
		 * later in this function — compute it from GBT nbit here so the marker is not
		 * stamped with 0 (explorer "Winning share difficulty" was blank/zero on SHA). */
		if (wb->nbit[0] && wb->network_diff <= 0.0) {
			uchar nbitsbin[4];

			if (hex2bin(nbitsbin, wb->nbit, 4))
				wb->network_diff = diff_from_nbits((char *)nbitsbin);
		}
		uint8_t marker[HOBC_MARKER_MAX];
		int mlen = hobc_build_marker(marker, ckp->hobc_pool_id, ckp->hobc_algo,
					     (float)wb->network_diff, ckp->hobc_pool_site,
					     NULL, NULL);
		if (mlen > 0 && mlen <= HOBC_MARKER_MAX) {
			wb->coinb2bin[wb->coinb2len++] = (uchar)mlen;
			memcpy(wb->coinb2bin + wb->coinb2len, marker, mlen);
			wb->coinb2len += mlen;
		}
	} else if (ckp->btcsig) {
...
/* Replace the pool treasury scriptPubKey in coinb2 with a miner's script. */
static bool patch_coinb2_payout(uchar *coinb2bin, int *coinb2len,
				const uchar *old_script, int old_len,
				const uchar *new_script, int new_len)
{
	int i, len_byte_pos = -1, script_pos = -1;

	if (!coinb2bin || !coinb2len || !old_script || !new_script || old_len <= 0 || new_len <= 0)
		return false;

	for (i = 0; i <= *coinb2len - old_len - 1; i++) {
		if (coinb2bin[i] == old_len && !memcmp(coinb2bin + i + 1, old_script, old_len)) {
			len_byte_pos = i;
			script_pos = i + 1;
			break;
		}
	...
	if (new_len != old_len) {
		int tail = *coinb2len - (script_pos + old_len);

		memmove(coinb2bin + script_pos + new_len, coinb2bin + script_pos + old_len, tail);
		*coinb2len += new_len - old_len;
	}
	coinb2bin[len_byte_pos] = new_len;
	memcpy(coinb2bin + script_pos, new_script, new_len);
	return true;
}

static bool build_client_coinb2(const workbase_t *wb, sdata_t *sdata,
				const stratum_instance_t *client,
				uchar *coinb2bin, int *coinb2len)
{
	ckpool_t *ckp;
	const user_instance_t *user;
	char worker[HOBC_MARKER_WORKER_LEN];
	bool touched = false;

	if (!wb || !sdata || !client || !coinb2bin || !coinb2len)
		return false;
	ckp = wb->ckp;
	user = client->user_instance;

	memcpy(coinb2bin, wb->coinb2bin, wb->coinb2len);
	*coinb2len = wb->coinb2len;

	if (ckp->direct_worker_payout && user && user->payout_script_ok &&
	    user->payout_script_len > 0) {
		if (patch_coinb2_payout(coinb2bin, coinb2len,
					(const uchar *)sdata->pubkeytxnbin, sdata->pubkeytxnlen,
					user->payout_script, user->payout_script_len))
			touched = true;
	}

	if (ckp->hobc_marker) {
		hobc_marker_worker_label(worker, sizeof(worker), client->workername);
		if (hobc_coinb2_patch_identity(coinb2bin, *coinb2len, NULL, worker) > 0)
			touched = true;
	}
...
retry:
	...
	/* During a GPU/KawPow round the SHA pool is intentionally idle: the
	 * generator returns an empty base with no json/flags. Building a coinbase
	 * from it would dereference NULL (generate_coinbase -> strlen(NULL)).
	 * Release it and skip this update, leaving miners on the last SHA base so
	 * they stay connected and resume instantly when SHA mining returns. */
	if (wb->sha_idle_gpu_window) {
		/* GPU/KawPow window: stop SHA miners wasting power. On window entry,
		 * drop all SHA clients once so their ASICs stop hashing the last SHA
		 * job; init_client()/update_client() then withhold work from any
		 * reconnects so they sit idle until SHA mining resumes. */
		if (!sdata->gpu_idle_window) {
			sdata->gpu_idle_window = true;
			LOGNOTICE("Entering GPU KawPow window; dropping SHA miners to stop wasted hashing");
			drop_allclients(ckp);
		} else
			LOGINFO("SHA pool idle during GPU KawPow round; skipping base update");
		dealloc(wb);
		ret = true;
		goto out;
	}

	/* Real SHA base obtained. If we were idle for a GPU window, SHA mining
	 * has resumed: clear the flag so (re)connecting miners receive work again
	 * and the stratum_broadcast_update() below pushes the fresh SHA job. */
	if (unlikely(sdata->gpu_idle_window)) {
		sdata->gpu_idle_window = false;
		LOGNOTICE("GPU KawPow window ended; resuming SHA mining and broadcasting fresh work");
	}
	...
	/* Always clean_jobs=true. Stock AxeOS / ESP-Miner (e.g. bitaxe v2.14.1)
	 * mishandles clean=false: it dequeues the notify but skips reprogramming
	 * the ASIC, so the previous nonce keeps being re-submitted as Duplicate
	 * until a clean=true job arrives (ESP-Miner #1587). Public-Pool / Ocean
	 * style clean=true on every notify avoids that class of reject storms. */
	stratum_broadcast_update(sdata, wb, true);
	ret = true;
...
share_diff(char *coinbase, const uchar *enonce1bin, const workbase_t *wb, const char *nonce2,
	   ...
	   const uchar *custom_coinb2bin, int custom_coinb2len)
{
	...
	char data[80];
	int i, coinb2len;
	const uchar *coinb2src;

	coinb2len = custom_coinb2bin ? custom_coinb2len : wb->coinb2len;
	coinb2src = custom_coinb2bin ? custom_coinb2bin : wb->coinb2bin;
	...
	*cblen += wb->enonce2varlen;
	memcpy(coinbase + *cblen, coinb2src, coinb2len);
	*cblen += coinb2len;
	...
	if (use_version_bits) {
		...
		/* HOBC V6: the SHA base block version is 0x30000000 (bit 28 + bit
		 * 29). Bit 28 (0x10000000) falls INSIDE the standard BIP320
		 * version-rolling domain 0x1fffe000, but ASIC firmware (Bitaxe /
		 * ESP-Miner, cgminer, etc.) overlays the rolled bits onto the full
		 * job version and submits only the rolled portion in version_bits
		 * (it does NOT re-supply bit 28). Rebuilding the header with
		 * (base & ~mask) | (bits & mask) therefore DROPPED bit 28, so the
		 * pool hashed 0x24xxxxxx while the miner hashed 0x34xxxxxx and every
		 * version-rolled share failed validation as "Above target". OR the
		 * rolled bits onto the base version so the pool reproduces exactly
		 * what the miner hashed (base bits 28/29 always preserved). */
		rolled_version = base_version | (version_bits & 0x1fffe000U);
...
static void submit_node_block(ckpool_t *ckp, sdata_t *sdata, json_t *val)
{
	...
	if (coinbasehex && cblen && swaphex) {
		...
		/* Fill in the hashes */
		share_diff(coinbase, enonce1bin, wb, nonce2, ntime32, nonce, false, 0, hash, swap, &cblen, NULL, 0);
	}
...
static double worker_estimated_hashrate_hs(worker_instance_t *worker);
static bool pool_ip_banned(ckpool_t *ckp, const char *address);
static void kick_client_for_ban(ckpool_t *ckp, sdata_t *sdata, stratum_instance_t *client);
...
/* Loose sanity check for a Bitcoin mainnet address (bech32 bc1.. or base58
 * P2PKH/P2SH 1../3..). Used only to recognise the embedded BTC field in
 * HOBCADDR.BTCADDR.WORKER so it can be stripped from the displayed worker. */
static bool looks_like_btc_address(const char *addr)
{
	size_t len;
...
/* If workername is HOBCADDR.BTCADDR.WORKER (the middle field looking like a BTC
 * address), rewrite it to HOBCADDR.WORKER into out and return true. Otherwise
 * leave it unchanged and return false. Keeps SHA-pool worker display normal and
 * backward compatible with legacy HOBCADDR.WORKER / HOBCADDR usernames. */
static bool hobc_collapse_btc_workername(const char *workername, char *out, size_t len)
{
	const char *first_dot, *field2start, *second_dot;
	char field2[128] = {};
	size_t flen;

	if (!workername || !len)
		return false;
	first_dot = strchr(workername, '.');
	if (!first_dot)
		return false;
	field2start = first_dot + 1;
	second_dot = strchr(field2start, '.');
	if (!second_dot)
		return false;
	flen = second_dot - field2start;
	if (flen >= sizeof(field2))
		flen = sizeof(field2) - 1;
	memcpy(field2, field2start, flen);
	field2[flen] = '\0';
	if (!looks_like_btc_address(field2))
		return false;
	snprintf(out, len, "%.*s%s", (int)(first_dot - workername), workername, second_dot);
	return true;
}
...
static void cache_user_payout_script(ckpool_t *ckp, user_instance_t *user)
{
	if (!ckp->direct_worker_payout || !user || user->payout_script_ok)
		return;
	if (!looks_like_hobc_bech32(user->username) &&
	    !generator_checkaddr(ckp, user->username))
		return;
	if (generator_load_address_script(ckp, user->username,
					  (char *)user->payout_script, &user->payout_script_len)) {
		user->payout_script_ok = true;
		LOGNOTICE("Direct payout script cached for user %s (%d bytes)",
			  user->username, user->payout_script_len);
	} else
		LOGWARNING("Failed to load payout script for user %s", user->username);
}
...
static void reload_max_hashrate_limit(ckpool_t *ckp);
static void reload_pool_bans(ckpool_t *ckp);
static void kick_banned_clients(ckpool_t *ckp, sdata_t *sdata);
static int reload_pool_bans_and_kick(ckpool_t *ckp, sdata_t *sdata);
...
retry:
	...
	if (cmdmatch(buf, "reloadmaxhashrate")) {
		char msg[128];

		reload_max_hashrate_limit(ckp);
		snprintf(msg, sizeof(msg), "{\"ok\":true,\"max_hashrate_ths\":%.3f}", ckp->max_hashrate_ths);
		send_unix_msg(umsg->sockd, msg);
		goto retry;
	}
	if (cmdmatch(buf, "reloadpoolbans")) {
		char msg[128];
		int ban_count = reload_pool_bans_and_kick(ckp, sdata);

		snprintf(msg, sizeof(msg), "{\"ok\":true,\"ban_count\":%d}", ban_count);
		send_unix_msg(umsg->sockd, msg);
		goto retry;
	}
	if (cmdmatch(buf, "listclients")) {
		json_t *val = NULL, *client_arr = json_array();
		stratum_instance_t *client;
		char *msg;
...
static void read_userstats(ckpool_t *ckp, sdata_t *sdata, int tvsec_diff)
{
	...
	while ((dir = readdir(d)) != NULL) {
		...
		json_get_int64(&worker->shares, val, "shares");
		{
			...
			if (json_get_int(&lastupdate, val, "lastupdate") && lastupdate > 0) {
				worker->last_share.tv_sec = lastupdate;
				worker->last_share.tv_usec = 0;
			} else {
				memset(&worker->last_share, 0, sizeof(worker->last_share));
			}
		}
				      ...
				      const char *workername)
{
	...
	cache_user_payout_script(ckp, user);
			       ...
			       json_t **err_val, int *errnum)
{
	...
	char effective_workername[256];
	const char *buf, *pass, *wname;
	bool ret = false;
	int arr_size;
	ts_t now;
	...
	if (pool_ip_banned(ckp, client->address)) {
		*err_val = json_string("Pool access unavailable");
		client->dropped = true;
		LOGNOTICE("Rejected auth for banned IP %s worker %s",
			  client->address, json_string_value(json_array_get(params_val, 0)) ?: "");
		goto out;
	}
	buf = json_string_value(json_array_get(params_val, 0));
	if (!buf) {
	...
	if (ckp->enforce_worker_address) {
		...
		bool bech32_prefix_ok = prefixlen &&
			!strncmp(worker_address, ckp->worker_address_prefix, prefixlen);
		/* Accept any address the node validates (hobc1… bech32 or H…
		 * legacy P2PKH/P2SH). Do not require the bech32-only prefix —
		 * that incorrectly forced legacy miners onto the pool address. */
		bool addr_ok = generator_checkaddr(ckp, worker_address);
		if (!addr_ok && bech32_prefix_ok && looks_like_hobc_bech32(worker_address))
			addr_ok = true;

		if (!addr_ok) {
			/* Invalid HOBC payout address. If configured, default to the
			 * pool's own address instead of rejecting the miner. */
			if (ckp->fallback_worker_address && ckp->btcaddress) {
				const char *dot = strchr(buf, '.');

				if (dot)
					snprintf(effective_workername, sizeof(effective_workername), "%s%s", ckp->btcaddress, dot);
				else
					snprintf(effective_workername, sizeof(effective_workername), "%s", ckp->btcaddress);
				LOGNOTICE("Worker %s has invalid HOBC address; defaulting to pool address", buf);
				wname = effective_workername;
			} else {
				*err_val = json_string(bech32_prefix_ok ?
					"Invalid HOBC payout address in workername" :
					"Invalid HOBC address in workername");
				goto out;
			}
		}
	}
	/* Strip the embedded BTC address field for normal SHA-pool worker display
	 * (HOBCADDR.BTCADDR.WORKER -> HOBCADDR.WORKER). */
	if (ckp->strip_btc_address_field) {
		char collapsed[256];

		if (hobc_collapse_btc_workername(wname, collapsed, sizeof(collapsed))) {
			memcpy(effective_workername, collapsed, sizeof(effective_workername) < sizeof(collapsed) ? sizeof(effective_workername) : sizeof(collapsed));
			effective_workername[sizeof(effective_workername) - 1] = '\0';
			wname = effective_workername;
		}
	}
	pass = json_string_value(json_array_get(params_val, 1));
	user = generate_user(ckp, client, wname);
	client->user_id = user->id;
	/* Max hashrate is enforced on live shares and by admin auto-ban — not at
	 * auth, because worker_instance can carry stale dsps from prior sessions. */
	ts_realtime(&now);
	client->start_time = now.tv_sec;
	/* NOTE workername is NULL prior to this so should not be used in code
	 * till after this point */
	client->workername = strdup(wname);
	/* Keep the miner-facing authorize string. fallback_worker_address and
	 * strip_btc_address_field may rewrite wname for payout/display, but
	 * firmware continues to send the original name on every mining.submit. */
	free(client->auth_workername);
	client->auth_workername = strdup(buf);
	if (pass)
...
static void reload_max_hashrate_whitelist(ckpool_t *ckp, json_t *json_conf)
{
	json_t *arr;
	int i, size;
	const char *ip;
	...
	if (!ckp)
		return;
	ckp->max_hashrate_whitelist_count = 0;
	if (!json_conf)
		return;
	arr = json_object_get(json_conf, "max_hashrate_whitelist_ips");
	if (!arr || !json_is_array(arr))
		return;
	size = json_array_size(arr);
	for (i = 0; i < size && ckp->max_hashrate_whitelist_count < MAX_HASHRATE_WHITELIST; i++) {
		json_t *entry = json_array_get(arr, i);

		ip = NULL;
		if (json_is_string(entry))
			ip = json_string_value(entry);
		if (!ip || !strlen(ip))
			continue;
		strncpy(ckp->max_hashrate_whitelist[ckp->max_hashrate_whitelist_count], ip,
			sizeof(ckp->max_hashrate_whitelist[0]) - 1);
		ckp->max_hashrate_whitelist[ckp->max_hashrate_whitelist_count]
			[sizeof(ckp->max_hashrate_whitelist[0]) - 1] = '\0';
		ckp->max_hashrate_whitelist_count++;
	}
}

static bool max_hashrate_ip_whitelisted(ckpool_t *ckp, const char *address)
{
	int i;

	if (!ckp || !address || !strlen(address))
		return false;
	for (i = 0; i < ckp->max_hashrate_whitelist_count; i++) {
		if (!strcmp(ckp->max_hashrate_whitelist[i], address))
			return true;
	}
	return false;
}
...
static void reload_max_hashrate_limit(ckpool_t *ckp)
{
	...
	if (json_conf) {
		if (json_get_double(&limit, json_conf, "max_hashrate_ths"))
			ckp->max_hashrate_ths = limit;
		reload_max_hashrate_whitelist(ckp, json_conf);
		json_decref(json_conf);
	}
	if (ckp->max_hashrate_ths <= 0.0 && ckp->name && !strcmp(ckp->name, "hobbyhash-ckpool-main"))
		ckp->max_hashrate_ths = 99999.0;
	LOGINFO("%s max miner hashrate limit now %.1f TH/s (%d whitelist IPs)",
		ckp->name ? ckp->name : "pool", ckp->max_hashrate_ths, ckp->max_hashrate_whitelist_count);
}

#define POOL_BAN_MAX 512
...
static pool_ban_entry_t pool_bans[POOL_BAN_MAX];
static int pool_bans_count;
static time_t pool_bans_loaded;
static time_t pool_bans_file_mtime;

static void reload_pool_bans(ckpool_t *ckp)
{
	json_t *val, *arr, *entry;
	...
	if (!ckp->ban_file || !strlen(ckp->ban_file)) {
		if (ckp->name && strstr(ckp->name, "nano"))
			ckp->ban_file = strdup("/home/hobbyhashcoin/hobbyhash-run/pool-bans-nano.json");
		else
			ckp->ban_file = strdup("/home/hobbyhashcoin/hobbyhash-run/pool-bans-main.json");
	}

	if (stat(ckp->ban_file, &st) == 0) {
		if (pool_bans_loaded && pool_bans_file_mtime == (time_t)st.st_mtime)
			return;
		pool_bans_file_mtime = (time_t)st.st_mtime;
	} else if (pool_bans_loaded && now - pool_bans_loaded < 15) {
		return;
	}

	pool_bans_loaded = now;
	pool_bans_count = 0;

	val = json_load_file(ckp->ban_file, 0, NULL);
	if (!val)
		return;
	arr = json_object_get(val, "bans");
	if (!arr || !json_is_array(arr))
		goto out;
	size = json_array_size(arr);
	for (i = 0; i < size && pool_bans_count < POOL_BAN_MAX; i++) {
		pool_ban_entry_t *ban = &pool_bans[pool_bans_count];
		char *ip = NULL;
		int expires = 0;
		...
		if (!entry || !json_is_object(entry))
			continue;
		if (!json_get_string(&ip, entry, "ip") || !ip || !strlen(ip))
			continue;
		strncpy(ban->ip, ip, sizeof(ban->ip) - 1);
		free(ip);
		ban->expires_at = 0;
		if (json_get_int(&expires, entry, "expires_at") && expires > 0)
			ban->expires_at = (time_t)expires;
...
static bool pool_ip_banned(ckpool_t *ckp, const char *address)
{
	...
	if (!address || !strlen(address))
		return false;
	for (i = 0; i < pool_bans_count; i++) {
		if (strcmp(pool_bans[i].ip, address))
			continue;
		if (pool_bans[i].expires_at && pool_bans[i].expires_at <= now)
			continue;
		return true;
	}
	return false;
...
static void kick_client_for_ban(ckpool_t *ckp, sdata_t *sdata, stratum_instance_t *client)
{
	if (client->dropped || client->reject >= 3)
		return;
	LOGNOTICE("Client %s worker %s blocked IP %s, disconnecting",
		  client->identity, client->workername ? client->workername : "",
		  client->address);
	stratum_send_message(sdata, client, "Disconnected from pool.");
	client->dropped = true;
	client->reject = 3;
...
static void kick_banned_clients(ckpool_t *ckp, sdata_t *sdata)
{
	...
	while (client) {
		if (!client->dropped && client->authorised && client->address && strlen(client->address)) {
			for (i = 0; i < pool_bans_count; i++) {
				if (strcmp(pool_bans[i].ip, client->address))
					continue;
				if (pool_bans[i].expires_at && pool_bans[i].expires_at <= now)
					break;
				kick_client_for_ban(ckp, sdata, client);
				break;
			}
		}
...
{
	pool_bans_loaded = 0;
	pool_bans_file_mtime = 0;
	reload_pool_bans(ckp);
	kick_banned_clients(ckp, sdata);
	return pool_bans_count;
}
...
static double worker_estimated_hashrate_hs(worker_instance_t *worker)
{
	...
	if (!worker)
		return 0;
	tv_time(&now);
	if (!worker->last_share.tv_sec || tvdiff(&now, &worker->last_share) > 120.0)
		return 0;
	dsps = worker->dsps1;
	if (worker->dsps5 > dsps)
		dsps = worker->dsps5;
	return dsps * nonces;
}

static double connection_hashrate_hs(stratum_instance_t *client)
{
	double dsps = client->dsps5;

	if (client->dsps1 > dsps)
		dsps = client->dsps1;
	return dsps * nonces;
}

/* Per-connection rate only — worker aggregates persist across reconnects and spike falsely. */
static double client_enforcement_hashrate_hs(stratum_instance_t *client, tv_t *now_t)
{
	(void)now_t;
	return connection_hashrate_hs(client);
}

static void log_hashrate_limit_event(ckpool_t *ckp, stratum_instance_t *client, double ths, const char *action)
{
	FILE *fp;
	const char *pool_name;
	time_t now = time(NULL);

	if (!ckp || !client || !action)
		return;
	pool_name = (ckp->name && strlen(ckp->name)) ? ckp->name : "pool";
	fp = fopen("/home/hobbyhashcoin/hobbyhash-run/pool-hashrate-limit-events.jsonl", "a");
	if (!fp)
		return;
	fprintf(fp,
		"{\"ts\":%ld,\"pool\":\"%s\",\"action\":\"%s\",\"limit_ths\":%.3f,\"estimated_ths\":%.6f,\"ip\":\"%s\",\"worker\":\"%s\",\"client_id\":%ld}\n",
		(long)now, pool_name, action, ckp->max_hashrate_ths, ths,
		client->address ? client->address : "",
		client->workername ? client->workername : "",
		(long)client->id);
	fclose(fp);
}

static void queue_hashrate_auto_ban(ckpool_t *ckp, stratum_instance_t *client, double ths)
{
	FILE *fp;
	const char *pool_name;
	const char *ip;
	...
	if (!ip || !strlen(ip))
		return;
	pool_name = (ckp->name && strlen(ckp->name)) ? ckp->name : "pool";
	fp = fopen("/home/hobbyhashcoin/hobbyhash-run/pool-hashrate-auto-ban-queue.jsonl", "a");
	if (!fp)
		return;
	fprintf(fp,
		"{\"ts\":%ld,\"pool\":\"%s\",\"ip\":\"%s\",\"worker\":\"%s\",\"limit_ths\":%.3f,\"estimated_ths\":%.6f,\"client_id\":%ld}\n",
		(long)now, pool_name, ip,
		client->workername ? client->workername : "",
		ckp->max_hashrate_ths, ths, (long)client->id);
	fclose(fp);
}

static void kick_client_for_hashrate(ckpool_t *ckp, sdata_t *sdata, stratum_instance_t *client,
				     double hs)
{
	char msg[160];
	double ths = hs / 1e12;
	const char *ip = client->address ? client->address : "";

	if (client->dropped || client->reject >= 3)
		return;

	if (max_hashrate_ip_whitelisted(ckp, ip))
		return;

	LOGNOTICE("Client %s IP %s worker %s exceeds max hashrate %.1f TH/s (est %.2f TH/s), disconnecting",
		  client->identity, ip, client->workername, ckp->max_hashrate_ths, ths);
	log_hashrate_limit_event(ckp, client, ths, "disconnect");
	snprintf(msg, sizeof(msg),
		 "Disconnected: hashrate exceeds pool maximum (%.0f TH/s).",
		 ckp->max_hashrate_ths);
	stratum_send_message(sdata, client, msg);
	client->dropped = true;
	client->reject = 3;
	connector_drop_client(ckp, client->id);
}

static void enforce_max_hashrate(ckpool_t *ckp, sdata_t *sdata, stratum_instance_t *client,
				 tv_t *now_t)
{
	double max_hs, hs;

	if (ckp->max_hashrate_ths <= 0.0)
		return;
	if (client->dropped || client->reject >= 3 || !client->authorised)
		return;
	if (max_hashrate_ip_whitelisted(ckp, client->address))
		return;
	if (!client->first_share.tv_sec || sane_tdiff(now_t, &client->first_share) < 60.0)
		return;

	max_hs = ckp->max_hashrate_ths * 1e12;
	hs = client_enforcement_hashrate_hs(client, now_t);
	if (hs <= max_hs)
		return;

	kick_client_for_hashrate(ckp, sdata, client, hs);
}
		       ...
		       const bool submit)
{
	...
	if (valid)
		enforce_max_hashrate(ckp, sdata, client, &now_t);
	if (client->reject >= 3)
		return;
...
{
	char *coinbase;
	uchar swap[80], client_coinb2bin[256];
	const uchar *custom_coinb2 = NULL;
	int custom_coinb2len = 0;
	ckpool_t *ckp = wb->ckp;
	sdata_t *sdata = client->sdata;
	double ret;
	int cblen;

	coinbase = ckalloc(wb->coinb1len + wb->enonce1constlen + wb->enonce1varlen + wb->enonce2varlen + wb->coinb2len + 32);

	if ((ckp->direct_worker_payout || ckp->hobc_marker) &&
	    build_client_coinb2(wb, sdata, client, client_coinb2bin, &custom_coinb2len))
		custom_coinb2 = client_coinb2bin;

	/* Calculate the diff of the share here */
	ret = share_diff(coinbase, client->enonce1bin, wb, nonce2, ntime32, nonce, use_version_bits,
			 version_bits, hash, swap, &cblen, custom_coinb2, custom_coinb2len);
...
/* Stock AxeOS / LuckyMiner re-read the same ASIC nonce until the job is
 * reprogrammed. Used to soft-ack Duplicate resubmits and refresh work. */
static bool client_sticky_asic_agent(const stratum_instance_t *client)
{
	const char *ua = client->useragent;

	if (!ua || !ua[0])
		return false;
	if (strcasestr(ua, "bitaxe/"))
		return true;
	if (strcasestr(ua, "luckyminer"))
		return true;
	return false;
}
			    ...
			    const json_t *params_val, json_t **err_val)
{
	...
	if (safecmp(workername, client->workername)) {
		/* strip_btc_address_field collapses HOBCADDR.BTCADDR.WORKER ->
		 * HOBCADDR.WORKER at authorise, but miners keep submitting the full
		 * uncollapsed name. Accept a submit whose collapsed form matches the
		 * stored (already collapsed) workername before treating it as a
		 * mismatch. Non-collapsing names and other pools are unaffected. */
		char collapsed_wn[256];
		bool ok = false;

		if (client->ckp->strip_btc_address_field &&
		    hobc_collapse_btc_workername(workername, collapsed_wn, sizeof(collapsed_wn)) &&
		    !safecmp(collapsed_wn, client->workername))
			ok = true;
		/* fallback_worker_address rewrites invalid ADDR.WORKER ->
		 * poolADDR.WORKER at authorise; miner still submits ADDR.WORKER.
		 * Accept the original authorize string we stored for this session. */
		if (!ok && client->auth_workername &&
		    !safecmp(workername, client->auth_workername))
			ok = true;
		if (!ok) {
...
out_nowb:
	...
	if (!invalid) {
		...
		if (sdiff >= diff) {
			if (new_share(sdata, hash, id)) {
				...
				result = true;
				/* Hint for sshare_process (stripped before miner sees it). */
				if (client_sticky_asic_agent(client))
					json_object_set_new_nocheck(json_msg, "_sticky_refresh",
								    json_true());
			} else {
				err = SE_DUPE;
				submit = false;
				/* AxeOS/LuckyMiner re-submit the same latched nonce ~10–15×.
				 * Soft-ack so miner UI / pool reject% stay sane; no double credit. */
				if (client_sticky_asic_agent(client)) {
					result = true;
					LOGDEBUG("Soft-ack sticky ASIC dupe from %s %s",
						 client->identity, client->useragent);
				} else {
					json_set_string(json_msg, "reject-reason", SHARE_ERR(err));
	...
	/* Sticky-ASIC duplicate soft-acks must not inflate accept or reject stats. */
	if (!(err == SE_DUPE && result && client_sticky_asic_agent(client)))
...
out:
	/* Duplicate resubmits from sticky ASIC firmwares are not "invalid shares"
	 * for disconnect purposes — the first copy was already accepted. */
	if (!sdata->wbincomplete && err != SE_DUPE && ((!result && !submit) || !share)) {
...
static json_t *__stratum_notify_client(const workbase_t *wb, const stratum_instance_t *client,
				       sdata_t *sdata, const bool clean)
{
	json_t *val;
	char *coinb2hex = NULL;
	const char *coinb2 = wb->coinb2;

	if (client) {
		uchar coinb2bin[256];
		int coinb2len = 0;

		if (build_client_coinb2(wb, sdata, client, coinb2bin, &coinb2len)) {
			coinb2hex = bin2hex(coinb2bin, coinb2len);
			coinb2 = coinb2hex;
		}
	}
...
static void stratum_broadcast_update_direct(sdata_t *sdata, const workbase_t *wb, const bool clean)
{
	ckpool_t *ckp = sdata->ckp;
	sdata_t *ckp_sdata = ckp->sdata;
	stratum_instance_t *client, *tmp;
	...
	HASH_ITER(hh, ckp_sdata->stratum_instances, client, tmp) {
		...
		json_msg = __stratum_notify_client(wb, client, sdata, clean);
		client_msg = ckalloc(sizeof(ckmsg_t));
		msg = ckzalloc(sizeof(smsg_t));
		if (subclient(client->id))
			json_set_string(json_msg, "node.method", stratum_msgs[SM_UPDATE]);
		msg->json_msg = json_msg;
		msg->client_id = client->id;
		client_msg->data = msg;
		DL_APPEND(bulk_send, client_msg);
...
static void stratum_broadcast_update(sdata_t *sdata, const workbase_t *wb, const bool clean)
{
	...
	if (ckp->direct_worker_payout || ckp->hobc_marker) {
		stratum_broadcast_update_direct(sdata, wb, clean);
		return;
	}
...
static void stratum_send_update(sdata_t *sdata, const int64_t client_id, const bool clean)
{
	...
	if (ckp->direct_worker_payout || ckp->hobc_marker) {
		ck_rlock(&sdata->instance_lock);
		HASH_FIND_I64(sdata->stratum_instances, &client_id, client);
		json_msg = __stratum_notify_client(sdata->current_workbase, client, sdata, clean);
		ck_runlock(&sdata->instance_lock);
	} else
...
static void update_client(const stratum_instance_t *client, const int64_t client_id)
{
	...
	/* Withhold SHA work while the chain is in a GPU/KawPow window so miners
	 * stay connected but idle instead of burning power on a job they cannot
	 * win. They resume automatically when SHA mining returns. */
	if (unlikely(sdata->gpu_idle_window)) {
		stratum_send_diff(sdata, client);
		return;
	}
...
static void sshare_process(ckpool_t *ckp, json_params_t *jp)
{
	...
	accepted = json_is_true(result_val);
	json_object_set_new_nocheck(json_msg, "result", result_val);
	json_object_set_new_nocheck(json_msg, "error", err_val ? err_val : json_null());
	steal_json_id(json_msg, jp);
	/* After a credited accept from sticky ASIC firmware, push clean work
	 * and (rate-limited) a fresh workbase. Strip internal hint first. */
	if (accepted && json_is_true(json_object_get(json_msg, "_sticky_refresh"))) {
		static time_t last_sticky_gbt;
		time_t now_gbt = time(NULL);

		json_object_del(json_msg, "_sticky_refresh");
		update_client(client, client_id);
		if (now_gbt != last_sticky_gbt) {
			last_sticky_gbt = now_gbt;
			update_base(sdata, GEN_PRIORITY);
		}
	} else
		json_object_del(json_msg, "_sticky_refresh");
...
static void *statsupdate(void *arg)
{
	...
	while (42) {
		static time_t last_hashrate_reload;
		...
		if (!last_hashrate_reload || now_sec - last_hashrate_reload >= 30)
			reload_max_hashrate_limit(ckp);
		last_hashrate_reload = now_sec;
		...
		while ((user = next_user(sdata, user)) != NULL) {
			...
			while ((worker = next_worker(sdata, user, worker)) != NULL) {
				...
				if (ckp->max_hashrate_ths > 0.0 && per_tdiff <= 120.0) {
					stratum_instance_t *wc;

					ck_rlock(&sdata->instance_lock);
					DL_FOREACH(user->clients, wc) {
						double chs;

						if (wc->worker_instance != worker || wc->dropped || !wc->authorised)
							continue;
						if (!wc->first_share.tv_sec || sane_tdiff(&now, &wc->first_share) < 60.0)
							continue;
						chs = client_enforcement_hashrate_hs(wc, &now);
						if (chs > ckp->max_hashrate_ths * 1e12)
							kick_client_for_hashrate(ckp, sdata, wc, chs);
					}
					ck_runlock(&sdata->instance_lock);
				}
				if (per_tdiff > 60) {
		...
		/* HOBC V6: emit the per-minute network census heartbeat to the node. */
		hobc_maybe_send_census(ckp, ghs1, stats->users + stats->remote_users,
				       stats->workers + stats->remote_workers, &now);
...
void *stratifier(void *arg)
{
	...
	reload_max_hashrate_limit(ckp);
	if (ckp->max_hashrate_ths > 0.0)
		LOGWARNING("%s max miner hashrate enforced at %.1f TH/s", ckp->name, ckp->max_hashrate_ths);
	if (ckp->direct_worker_payout)
		LOGWARNING("%s direct worker payout enabled — coinbase pays miner HOBC addresses (no payout daemon)", ckp->name);
	...
	if (!ckp->proxy) {
		...
		if (looks_like_hobc_bech32(ckp->btcaddress)) {
			if (!generator_load_address_script(ckp, ckp->btcaddress, sdata->pubkeytxnbin, &sdata->pubkeytxnlen)) {
				LOGEMERG("Fatal: failed to load scriptPubKey for bech32 btcaddress %s", ckp->btcaddress);
				goto out;
			}
		} else if (script_address(ckp->btcaddress)) {
