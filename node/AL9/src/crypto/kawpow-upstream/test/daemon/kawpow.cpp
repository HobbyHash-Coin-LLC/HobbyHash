...
int main(int argc, const char* argv[])
{
    ...
    void (*OnReq)(evhttp_request *req, void *) = [] (evhttp_request *req, void *)
    {
        ...
        if (!header_hash_str || !mix_hash_str || !nonce_str || !height_str || !share_boundary_str || !block_boundary_str) {
            ...
            if (mix_match == "true" && ethash::is_less_or_equal(result.final_hash, share_boundary)) {
                share_met = "true";
            }

            if (mix_match == "true" && ethash::is_less_or_equal(result.final_hash, block_boundary)) {
                block_met = "true";
            }

            evbuffer_add_printf(OutBuf, "{\"%s\":%s, \"%s\":%s, \"%s\":%s, \"%s\":\"%s\", \"mix_hash\":\"%s\"}", "result", mix_match.c_str(),
                                "share", share_met.c_str(), "block", block_met.c_str(), "digest",
                                to_hex(result.final_hash).c_str(), to_hex(result.mix_hash).c_str());
