...
template <class T>
uint256 SignatureHash(const CScript& scriptCode, const T& txTo, unsigned int nIn, int32_t nHashType, const CAmount& amount, SigVersion sigversion, const PrecomputedTransactionData* cache, SigHashCache* sighash_cache, uint32_t replay_fork_id)
{
    assert(nIn < txTo.vin.size());
    const bool fork_sighash = replay_fork_id != 0 && (nHashType & SIGHASH_FORKID);

    if (sigversion == SigVersion::WITNESS_V0 && fork_sighash) {
        ...
        ss << hashOutputs;
        ss << txTo.nLockTime;
        const int baseType = nHashType & ~SIGHASH_FORKID;
        ss << (replay_fork_id << 8 | (baseType & 0xff));
        return ss.GetHash();
    }
    ...
    if (!fork_sighash && sighash_cache && sighash_cache->Load(nHashType, scriptCode, ss)) {
    ...
    if (!fork_sighash && sighash_cache != nullptr) {
        sighash_cache->Store(nHashType, scriptCode, ss);
    }

    // Add sighash type and hash.
    ss << nHashType;
    if (fork_sighash && sigversion != SigVersion::WITNESS_V0) {
        ss << replay_fork_id;
    }
    return ss.GetHash();
}
...
bool GenericTransactionSignatureChecker<T>::CheckECDSASignature(const std::vector<unsigned char>& vchSigIn, const std::vector<unsigned char>& vchPubKey, const CScript& scriptCode, SigVersion sigversion) const
{
    ...
    if (m_require_fork_sighash && !(nHashType & SIGHASH_FORKID)) {
        return false;
    }
    ...
    uint256 sighash = SignatureHash(scriptCode, *txTo, nIn, nHashType, amount, sigversion, this->txdata, &m_sighash_cache, m_replay_fork_id);
