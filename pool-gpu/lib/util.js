...
var Keccak = require('sha3').Keccak;
...
exports.kawPowKeccak256 = function (buffer) {
    return new Keccak(256).update(buffer).digest();
};

exports.kawPowHeaderHashHex = function (rpcData, merkleRootHex, heightOverride) {
    var buf = Buffer.alloc(80);
    var off = 0;
    var height = typeof heightOverride === 'number' ? heightOverride : rpcData.height;
    var prevInternal = exports.reverseBuffer(Buffer.from(rpcData.previousblockhash, 'hex'));
    var merkleInternal = Buffer.from(merkleRootHex, 'hex');
    buf.writeUInt32LE(rpcData.version >>> 0, off);
    off += 4;
    prevInternal.copy(buf, off);
    off += 32;
    merkleInternal.copy(buf, off);
    off += 32;
    buf.writeUInt32LE(rpcData.curtime >>> 0, off);
    off += 4;
    buf.writeUInt32LE(parseInt(rpcData.bits, 16) >>> 0, off);
    off += 4;
    buf.writeUInt32LE(height >>> 0, off);
    return exports.kawPowKeccak256(buf).toString('hex');
};

exports.kawPowSeedHashHex = function (height) {
    var epochLength = 7500;
    var epoch = Math.floor(height / epochLength);
    var seed = Buffer.alloc(32, 0);
    var i;
    for (i = 0; i < epoch; i++) {
        seed = exports.kawPowKeccak256(seed);
    }
    return seed.toString('hex');
};

exports.kawPowEthTargetHex = function (difficulty) {
    // TBM's native eth_getWork path interprets target against Bitcoin-style
    // diff1, not KawPow's reduced pow limit used for pool share validation.
    var diff1 = bignum('00000000ffff0000000000000000000000000000000000000000000000000000', 16);
    var d = parseFloat(difficulty);
    if (!isFinite(d) || d <= 0) {
        d = 0.01;
    }
    var scale = 100000000;
    var dScaled = Math.max(1, Math.round(d * scale));
    var target = diff1.mul(scale).div(dScaled);
    var hex = target.toString(16);
    if (hex.length > 64) {
        hex = hex.substr(hex.length - 64);
    }
    while (hex.length < 64) {
        hex = '0' + hex;
    }
...
exports.KAWPOW_POW_LIMIT_HEX = '00000000ff000000000000000000000000000000000000000000000000000000';

exports.kawpowShareTargetHex = function (difficulty) {
    var powLimit = bignum(exports.KAWPOW_POW_LIMIT_HEX, 16);
    var d = parseFloat(difficulty);
    if (!isFinite(d) || d <= 0) {
        d = 0.01;
    }
    var scale = 100000000;
    var dScaled = Math.max(1, Math.round(d * scale));
    var target = powLimit.mul(scale).div(dScaled);
    var hex = target.toString(16);
    if (hex.length > 64) {
        hex = hex.substr(hex.length - 64);
    }
...
exports.kawpowNetworkDifficultyFromTargetHex = function (targetHex) {
    var normalized = String(targetHex || '').replace(/^0x/i, '');
    if (!normalized || /^0+$/.test(normalized)) {
        return 0.000000001;
    }
    var target = bignum(normalized, 16);
    if (target.cmp(0) === 0) {
        return 0.000000001;
    }
    var diff1 = bignum(exports.KAWPOW_POW_LIMIT_HEX, 16);
    var scale = bignum('1000000000000000000');
    var diffScaled = diff1.mul(scale).div(target);
    if (diffScaled.cmp(0) === 0) {
        return 0.000000001;
    }
    var diffStr = diffScaled.toString();
    if (diffStr.length > 18) {
        return parseFloat(diffStr.slice(0, -18) + '.' + diffStr.slice(-18));
    }
    return parseFloat('0.' + diffStr.padStart(18, '0'));
};
