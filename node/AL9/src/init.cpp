...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-lwmaretarget", "Regtest only: enable per-algo LWMA difficulty retargeting (default off). Used by the V6 multi-algo timing burn-in so difficulty floats with real miners.", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
#if HAVE_SYSTEM
    ...
    argsman.AddArg("-censustoken=<token>", "Shared secret required by the submitcensus RPC. HOBC-compliant pools present this token to push per-minute mining census heartbeats (hashrate, unique miners, workers). If unset, submitcensus is disabled.", ArgsManager::ALLOW_ANY | ArgsManager::SENSITIVE, OptionsCategory::RPC);
