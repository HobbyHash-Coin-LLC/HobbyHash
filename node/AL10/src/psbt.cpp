...
PSBTError SignPSBTInput(const SigningProvider& provider, PartiallySignedTransaction& psbt, int index, const PrecomputedTransactionData* txdata, std::optional<int> sighash,  SignatureData* out_sigdata, bool finalize, int sign_at_height)
{
    ...
    if (txdata == nullptr) {
        ...
        MutableTransactionSignatureCreator creator(tx, index, utxo.nValue, txdata, *sighash, replay_fork_id, require_fork_sighash);
