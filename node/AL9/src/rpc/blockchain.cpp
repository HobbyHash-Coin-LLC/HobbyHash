...
UniValue blockToJSON(BlockManager& blockman, const CBlock& block, const CBlockIndex& tip, const CBlockIndex& blockindex, TxVerbosity verbosity, const uint256 pow_limit, const Consensus::Params& params)
{
    (void)pow_limit; // kept for call-site compatibility; target uses per-algo limit via params
    ...
    // V6 telemetry: mining algorithm for this block (height-gated; historical blocks read as
    // sha256/kawpow per the pre-race schedule, never misclassified by rolled version bits).
    const PowAlgo algo = AlgoFromHeader(block, blockindex.nHeight, params);
    result.pushKV("powalgo", PowAlgoName(algo));

    // V6 telemetry: decode the HOBC coinbase marker if present (only emitted at/after activation).
    // Off-chain data (worker IPs) is never on-chain and is not exposed here.
    if (!block.vtx[0]->vin.empty()) {
        ...
        if (ParseHobcTelemetryMarker(cb_bytes, marker)) {
            ...
            m.pushKV("version", (int)HobcTelemetryMarker::VERSION);
            m.pushKV("pool_id", (int)marker.pool_id);
            m.pushKV("algo", (int)marker.algo);
            m.pushKV("algo_name", PowAlgoName(static_cast<PowAlgo>(marker.algo)));
            m.pushKV("winning_share_diff", (double)marker.winning_share_diff);
            if (!marker.pool_site.empty()) m.pushKV("pool_site", marker.pool_site);
            if (!marker.rig_model.empty()) m.pushKV("rig_model", marker.rig_model);
            if (!marker.worker_name.empty()) m.pushKV("worker_name", marker.worker_name);
            result.pushKV("hobc_marker", std::move(m));
        }
    }
    ...
    return RPCHelpMan{
        ...
        "If verbosity is 0, returns a string that is serialized, hex-encoded data for block 'hash'.\n"
                ...
                RPCResult::Type::OBJ, "", "",
                {
                    ...
                    {RPCResult::Type::STR_HEX, "previousblockhash", /*optional=*/true, "The hash of the previous block (if available)"},
                    {RPCResult::Type::STR_HEX, "nextblockhash", /*optional=*/true, "The hash of the next block (if available)"},
                    {RPCResult::Type::STR, "powalgo", "The mining algorithm for this block (sha256, kawpow, or randomx)"},
                    {RPCResult::Type::OBJ, "hobc_marker", /*optional=*/true, "Decoded HOBC coinbase telemetry marker (present at/after the multi-algo race activation height)",
                    {
                        {RPCResult::Type::NUM, "version", "Marker format version"},
                        {RPCResult::Type::NUM, "pool_id", "Numeric pool identifier assigned to the reporting pool"},
                        {RPCResult::Type::NUM, "algo", "Algo code (0=sha256, 1=kawpow, 2=randomx)"},
                        {RPCResult::Type::STR, "algo_name", "Algo name"},
                        {RPCResult::Type::NUM, "winning_share_diff", "Difficulty of the share that solved the block"},
                        {RPCResult::Type::STR, "pool_site", /*optional=*/true, "Pool website or address as shown on the explorer"},
                        {RPCResult::Type::STR, "rig_model", /*optional=*/true, "Miner/rig model (e.g. S19), if reported"},
                        {RPCResult::Type::STR, "worker_name", /*optional=*/true, "Worker name, if reported"},
                    }},
                }},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    CBlock block{};
    if (!DeserializeBlockFromBytes(block, block_data, pblockindex->nHeight, chainman.GetConsensus())) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Block deserialization failed");
    }
    ...
    return blockToJSON(chainman.m_blockman, block, *tip, *pblockindex, tx_verbosity, chainman.GetConsensus().powLimit, chainman.GetConsensus());
},
    };
}
