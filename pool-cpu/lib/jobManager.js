...
var JobManager = module.exports = function JobManager(options) {
  ...
  this.processShare = function (miner_given_jobId, previousDifficulty, difficulty, miner_given_nonce, ipAddress, port, workerName, miner_given_header, miner_given_mixhash, extraNonce1, callback_parent, shareOptions) {
    ...
    var header_hash = jobHeaderHash(job);
    ...
    if (miner_given_header && miner_given_header.length > 0 && header_hash != miner_given_header)
        ...
        function(callback) {
          ...
          res.on("end", () => {
            ...
            if (body.block === true) {
              ...
              blockHex = job.serializeBlock(header_hash, miner_given_nonce, submitMix).toString('hex');
      ...
      _this.daemon.cmd('getkawpowhash', [ header_hash, miner_given_mixhash, miner_given_nonce, job.rpcData.height, job.target_hex ], function (results) {
        ...
        if (result == 'true') {
          ...
          if (job.target.ge(headerBigNum)) {
            ...
            var blockHex = job.serializeBlock(header_hash, miner_given_nonce, mix_hash).toString('hex');
