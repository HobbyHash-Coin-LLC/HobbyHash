...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock()
{
    ...
    // Block subsidy + fees
    const CAmount block_reward{nFees + GetBlockSubsidy(nHeight, chainparams.GetConsensus(), pindexPrev)};
    ...
    // HOBC V6: at/after the multi-algo race activation height the coinbase must carry a
    // well-formed HOBC telemetry marker (consensus rule in ContextualCheckBlock). The node's
    // own assembler builds a SHA256 template (KawPow/RandomX algo bits are only added later by
    // getblocktemplate when a pool requests them, at which point the pool supplies its own
    // coinbase), so we stamp a SHA-algo marker here. This keeps getblocktemplate's internal
    // TestBlockValidity self-check and generatetoaddress valid post-activation. Real pools
    // replace this coinbase (and marker) with their own.
    if (MultiAlgoRaceActive(nHeight, chainparams.GetConsensus())) {
        HobcTelemetryMarker hobc_m;
        hobc_m.pool_id = 0;                                        // node/internal miner
        hobc_m.algo = static_cast<uint8_t>(PowAlgo::SHA256);       // assembler builds SHA
        hobc_m.winning_share_diff = 0.0f;                          // placeholder; pools stamp real diff
        const std::vector<unsigned char> hobc_bytes = BuildHobcTelemetryMarker(hobc_m);
        coinbaseTx.vin[0].scriptSig << std::span<const unsigned char>(hobc_bytes);
    }
