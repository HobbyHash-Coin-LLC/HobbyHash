...
MutableTransactionSignatureCreator::MutableTransactionSignatureCreator(const CMutableTransaction& tx, unsigned int input_idx, const CAmount& amount, int hash_type, uint32_t replay_fork_id, bool require_fork_sighash)
    : m_txto{tx}, nIn{input_idx}, nHashType{hash_type}, amount{amount},
      checker{&m_txto, nIn, amount, MissingDataBehavior::FAIL, replay_fork_id, require_fork_sighash},
      m_txdata(nullptr), m_replay_fork_id{replay_fork_id}, m_require_fork_sighash{require_fork_sighash}
{
}

MutableTransactionSignatureCreator::MutableTransactionSignatureCreator(const CMutableTransaction& tx, unsigned int input_idx, const CAmount& amount, const PrecomputedTransactionData* txdata, int hash_type, uint32_t replay_fork_id, bool require_fork_sighash)
    : m_txto{tx}, nIn{input_idx}, nHashType{hash_type}, amount{amount},
      checker{txdata ? MutableTransactionSignatureChecker{&m_txto, nIn, amount, *txdata, MissingDataBehavior::FAIL, replay_fork_id, require_fork_sighash} :
                       MutableTransactionSignatureChecker{&m_txto, nIn, amount, MissingDataBehavior::FAIL, replay_fork_id, require_fork_sighash}},
      m_txdata(txdata), m_replay_fork_id{replay_fork_id}, m_require_fork_sighash{require_fork_sighash}
{
}
...
bool MutableTransactionSignatureCreator::CreateSig(const SigningProvider& provider, std::vector<unsigned char>& vchSig, const CKeyID& address, const CScript& scriptCode, SigVersion sigversion) const
{
    ...
    const int hashtype = ReplayAwareSignHashType(nHashType, m_replay_fork_id);

    uint256 hash = SignatureHash(scriptCode, m_txto, nIn, hashtype, amount, sigversion, m_txdata, nullptr, m_replay_fork_id);
...
static bool PubKeyMatchesKeyID(const CPubKey& pubkey, const CKeyID& address)
{
    return pubkey.IsValid() && pubkey.GetID() == address;
}
...
static bool GetPubKey(const SigningProvider& provider, const SignatureData& sigdata, const CKeyID& address, CPubKey& pubkey)
{
    ...
    if (it != sigdata.signatures.end() && PubKeyMatchesKeyID(it->second.first, address)) {
        pubkey = it->second.first;
        return true;
    }
    // Look for pubkey in pubkey lists
    const auto& pk_it = sigdata.misc_pubkeys.find(address);
    if (pk_it != sigdata.misc_pubkeys.end() && PubKeyMatchesKeyID(pk_it->second.first, address)) {
        pubkey = pk_it->second.first;
        return true;
    }
    const auto& tap_pk_it = sigdata.tap_pubkeys.find(address);
    if (tap_pk_it != sigdata.tap_pubkeys.end()) {
        CPubKey candidate = tap_pk_it->second.GetEvenCorrespondingCPubKey();
        if (PubKeyMatchesKeyID(candidate, address)) {
            pubkey = candidate;
            return true;
        }
    }
    // Query the underlying provider
    CPubKey provider_pubkey;
    if (provider.GetPubKey(address, provider_pubkey) && PubKeyMatchesKeyID(provider_pubkey, address)) {
        pubkey = provider_pubkey;
        return true;
    }

    // Some restored/encrypted wallets can still have the private key even when
    // the pubkey cache used by the signer is missing or stale. Derive it only
    // when the key hash proves it is the requested key.
    CKey key;
    if (!provider.GetKey(address, key)) {
        return false;
    }
    CPubKey derived_pubkey = key.GetPubKey();
    if (!PubKeyMatchesKeyID(derived_pubkey, address)) {
        return false;
    }
...
bool SignTransaction(CMutableTransaction& mtx, const SigningProvider* keystore, const std::map<COutPoint, Coin>& coins, int nHashType, std::map<int, bilingual_str>& input_errors, int sign_at_height)
{
    bool fHashSingle = ((nHashType & ~SIGHASH_ANYONECANPAY) == SIGHASH_SINGLE);

    const Consensus::Params& consensus = Params().GetConsensus();
    uint32_t replay_fork_id = 0;
    bool require_fork_sighash = false;
    if (sign_at_height >= 0) {
        const auto replay = ReplayForkForHeight(sign_at_height, consensus);
        replay_fork_id = replay.fork_id;
        require_fork_sighash = replay.require_sighash;
    }
    ...
    for (unsigned int i = 0; i < mtx.vin.size(); ++i) {
        ...
        if (!fHashSingle || (i < mtx.vout.size())) {
            ProduceSignature(*keystore, MutableTransactionSignatureCreator(mtx, i, amount, &txdata, nHashType, replay_fork_id, require_fork_sighash), prevPubKey, sigdata);
        }
        ...
        if (!sigdata.complete && !VerifyScript(txin.scriptSig, prevPubKey, &txin.scriptWitness, STANDARD_SCRIPT_VERIFY_FLAGS, TransactionSignatureChecker(&txConst, i, amount, txdata, MissingDataBehavior::FAIL, replay_fork_id, require_fork_sighash), &serror)) {
